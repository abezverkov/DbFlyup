...
{
    public abstract class HashedTableJournal : IJournal
    {
        public class HashedTableJournalEntry
        {
            public HashedTableJournalEntry(string name, string hash)
            {
                Name = name;
                Hash = hash;
            }
        ...
        public virtual string[] GetExecutedScripts()
        {
            return ConnectionManager().ExecuteCommandsWithManagedConnection(dbCommandFactory =>
            {
                if (journalExists || DoesTableExist(dbCommandFactory))
                {
                    ...
                    var scripts = new List<HashedTableJournalEntry>();
                    ...
                    using (var command = GetJournalEntriesCommand(dbCommandFactory))
                    {
                        using (var reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                scripts.Add(new HashedTableJournalEntry(GetStringSafe(reader,0), GetStringSafe(reader, 1)));
                            }
                        }
                    }

                    return GetExecutedHashScripts(scripts).ToArray();
                }
                else
                {
        ...
        private static string GetStringSafe(IDataReader reader, int colIndex)
        {
            ...
            var val = reader[colIndex];
            if (val == null || val is DBNull) return null;

            return val.ToString();
        }

        protected IEnumerable<string> GetExecutedHashScripts(IEnumerable<HashedTableJournalEntry> journaledScripts)
        {
            var deploymentHashScripts = GetDeploymentHashScripts();

            foreach (var journaledScript in journaledScripts)
            {
                if (string.IsNullOrWhiteSpace(journaledScript.Hash))
                    yield return journaledScript.Name;
                if (deploymentHashScripts.Any(x => x.Name == journaledScript.Name && x.Hash == journaledScript.Hash))
                    yield return journaledScript.Name;
            }
        }

        protected abstract IEnumerable<HashedTableJournalEntry> GetDeploymentHashScripts();
        ...
        protected IDbCommand GetCreateSchemaCommand(Func<IDbCommand> dbCommandFactory)
        {
            var command = dbCommandFactory();
        ...
        /// <returns></returns>
        protected abstract string GetInsertJournalEntrySql(string @scriptName, string @applied, string @scriptHash);
        ...
        public virtual void EnsureTableExistsAndIsLatestVersion(Func<IDbCommand> dbCommandFactory)
        {
            if (!journalExists && !DoesTableExist(dbCommandFactory))
            {
                ...
                using (var command = GetCreateSchemaCommand(dbCommandFactory))
                {
                    command.ExecuteNonQuery();
                }
